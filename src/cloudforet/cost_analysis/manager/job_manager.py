...
class JobManager(BaseManager):
    ...
    def get_tasks(
        ...
        if secret_type == "MANUAL":
            ...
            billing_account_agreement_type = getattr(
                billing_account_info, "agreement_type", ""
            )
            ...
            if billing_account_agreement_type == "MicrosoftPartnerAgreement":
                ...
                customer_tenants = self._get_customer_tenants(secret_data)
    ...
    def _get_customer_tenants(self, secret_data: dict) -> list:
    ...
    @staticmethod
    def _get_divided_customer_tenants(customer_tenants: list) -> list:
        tenant_size = math.ceil(len(customer_tenants) / _TASK_LIST_SIZE)
        divided_customer_tenants = [
            customer_tenants[idx : idx + tenant_size]
            for idx in range(0, len(customer_tenants), tenant_size)
        ]
        return divided_customer_tenants
